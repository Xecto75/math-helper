...
- **Lecture façon diaporama (2026-07-31)** : une leçon ne joue plus d'un trait. `expandPageSegments` (App.jsx) découpe chaque page en *beats* — la pause vient UNIQUEMENT AVANT une étape qui EFFACE quelque chose de lisible : `tx/td/text-clear/cd/cx` (toujours destructives), ou une réécriture d’une box/comment vivante dont le nouveau texte ne CONTIENT pas l’ancien. Ajouter ne coupe jamais — `x = 1` qui devient `x = 1|y = -1` ne coupe pas, `x = 1` qui devient `x = 3` coupe. Comparaison champ par champ (titre vs corps) pour qu’un changement de titre ne passe pas pour un effacement du corps. Des étapes destructrices CÔTE À CÔTE ne comptent que pour UNE pause — supprimer un commentaire puis sa box, c’est un seul moment, pas deux clics. Plus chaque `set-layout`. Tout le reste (tracés, formes, `ef`) reste dans son beat : personne ne clique dix fois pour assembler une image. `‹ ›` traversent les beats ET les pages en continu. Cliquer « suivant » PENDANT qu’un beat joue ne saute pas : `setHurry(true)` (ActionExecutor) accélère ×10 le reste du beat — toutes les étapes s’exécutent quand même, sinon le beat suivant dessinerait sur un canvas auquel il manque des formes. `cancelAllAnimations()` remet hurry à false, donc chaque nouveau beat repart à vitesse normale, Espace/→/← et un clic sur le fond font pareil, la flèche `›` pulse quand un beat est fini. Le bouton pause a disparu (l'état `paused` reste, le Lesson Builder s'en sert pour son pas-à-pas). Les points de la barre = les PAGES, pas les beats.
